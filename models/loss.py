...
from models.archs.arch_util import DWT

class CharbonnierLoss(nn.Module):
    """Charbonnier Loss (L1) - Optimized for PSNR (Using Mean & eps^2)"""
...
class GradientLoss(nn.Module):
    ...
    def __init__(self, lambda1=1.0, lambda2=0.1, threshold=0.1, edge_logit_scale=12.0):
    ...
    def forward(self, pred_grad, gt_grad):
        l1_grad_loss = self.l1_loss(pred_grad, gt_grad)
        gt_edge = (gt_grad > self.threshold).float().detach()
        logits = (pred_grad - self.threshold) * self.edge_logit_scale
        ce_edge_loss = F.binary_cross_entropy_with_logits(logits, gt_edge, reduction='mean')
        
        total_loss = self.lambda1 * l1_grad_loss + self.lambda2 * ce_edge_loss
        return total_loss



class HighFrequencyMaskedLoss(nn.Module):
    
    def __init__(self, eps=1e-6, threshold=0.5):
        super(HighFrequencyMaskedLoss, self).__init__()
        self.threshold = threshold
        self.register_buffer('sobel_x', torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).view(1, 1, 3, 3))
        self.register_buffer('sobel_y', torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).view(1, 1, 3, 3))

    def compute_gradient_mag(self, img):
        b, c, h, w = img.shape
    ...
    def forward(self, pred, target):
        grad_target = self.compute_gradient_mag(target)
        grad_pred = self.compute_gradient_mag(pred)
        mask = (grad_target > self.threshold).float()
        diff = (grad_pred - grad_target).abs() * mask
