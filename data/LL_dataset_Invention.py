    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            for i in range(self.invention_num):
                ...
                # Apply the specific crop and augmentation for this random image
                img_color_invention, img_light_invention = img_color_invention[0], img_light_invention[0]
                
           
                _, c_H, c_W = img_color_invention.shape
           
                c_rnd_h = random.randint(0, max(0, c_H - GT_size))
                c_rnd_w = random.randint(0, max(0, c_W - GT_size))
                
             
                img_color_invention = img_color_invention[:, c_rnd_h:c_rnd_h + GT_size, c_rnd_w:c_rnd_w + GT_size]
                img_light_invention = img_light_invention[:, c_rnd_h:c_rnd_h + GT_size, c_rnd_w:c_rnd_w + GT_size]
                
              
                resize_op = transforms.Resize((GT_size, GT_size), antialias=True)
                img_color_invention = resize_op(img_color_invention)
                img_light_invention = resize_op(img_light_invention)
